...
class Finishedgood extends CI_Controller
{
	...
	public function index()
	{
		...
		if ($check_permission->num_rows() == 1) {
			...
			$data['nama_halaman'] 	= "Control Finished Good";
			...
			//END LOG
			$this->load->view('adminx/produksi/finishedgood/job', $data, FALSE);
		} else {
  ...
  public function list_data()
  {
    ...
    $Kategori             = ($Kategori === 'ALL') ? 'NULL' : "'$Kategori'";
    
    $Sql                  = "EXEC dbo.GetJobFinishGood @Tahun = '$Year', @Bulan = '$Month', @Kategori = $Kategori";
  ...
  public function lhp_calender()
	{
		...
		if ($check_permission->num_rows() == 1) {
			$data['group_halaman'] 	= "Produksi";
			$data['nama_halaman'] 	= "LHP Produksi";
			$data['icon_halaman'] 	= "icon-airplay";
			$data['perusahaan'] 		= $this->perusahaan->get_details();
			...
			log_helper($log_url, $log_type, $log_data);
			//END LOG
			$this->load->view('adminx/produksi/finishedgood/lhp_calendar', $data, FALSE);
		} else {
			redirect('errorpage/error403');
		}
	}

  public function lhp_calender_data()
  {
    $Draw                 = intval($this->input->get("draw"));
    $Start                = intval($this->input->get("start"));
    $Length               = intval($this->input->get("length"));
    ...
    $Sql                  = "EXEC dbo.GetJobFinishGoodKalender @Bulan = $Month, @Tahun = $Year";
    $Query                = $this->BJGMAS01->query($Sql);
    $Result               = $Query->result();
    $Data 	              = [];
    $No                   = 1;
    ...
    foreach ($Result as $key => $Res) {
      ...
      for ($i = 1; $i <= 31; $i++) {
        ...
        $Row[]    = isset($Res->$planKey) ? number_format($Res->$planKey, 0, '', '.') : '';
        $Row[]    = isset($Res->$actKey) ? number_format($Res->$actKey, 0, '', '.') : '';
      }
      $Row[]  = $Res->Notes;
      $Data[] = $Row;
    }
  ...
  public function save_data()
  {
    ...
    $checkExist = $this->BJGMAS01->get_where('Trans_JobFinishGoodHD', ['JobNumber' => $JobNumber]);
    ...
    // Simpan Header
    if (!$this->BJGMAS01->insert('Trans_JobFinishGoodHD', $FirstData)) {
      $this->BJGMAS01->trans_rollback();
      $this->responseJSON(500, 'error', 'Gagal menyimpan data header.', ['step' => 'Header Insert']);
    }

    // Simpan Detail 1
    if (!$this->BJGMAS01->insert_batch('Trans_JobFinishGoodDT', $SecondData)) {
      $this->BJGMAS01->trans_rollback();
  ...
  public function edit_data()
  {
    ...
    $this->BJGMAS01->select("
      a.Id, a.JobNumber, a.JobDate, a.PartID, b.PartName, a.Notes, a.UnitID, a.Kategori,
      REPLACE(FORMAT(a.JobQuantity, 'N0'), ',', '.') AS JobQuantity,
      FORMAT(a.JobDate, 'yyyy-MM') AS JobPeriode
    ");
    $this->BJGMAS01->from('Trans_JobFinishGoodHD a');
    ...
    if ($Query->num_rows() > 0) {
      ...
      ", false);
      $this->BJGMAS01->from("Trans_JobFinishGoodDT");
  ...
  public function update_data()
  {
    ...
    if ($this->BJGMAS01->update('Trans_JobFinishGoodHD', $FirstData, ['Id' => $FirstId])) {
      !empty($SecondDataInsert) && $this->BJGMAS01->insert_batch('Trans_JobFinishGoodDT', $SecondDataInsert);
      !empty($SecondDataUpdate) && $this->BJGMAS01->update_batch('Trans_JobFinishGoodDT', $SecondDataUpdate, 'Id');
  ...
  public function hapus_row_jumlah()
  {
    ...
    $Delete = $this->BJGMAS01->delete('Trans_JobFinishGoodDT', array('Id' => $KodeSecond, 'JobNumber' => $JobNumber));
    if ($Delete) {
      echo json_encode([
        "status_code" => 200,
        "status"      => "success",
  ...
  public function hapus_all()
  {
    ...
    $this->BJGMAS01->where('JobNumber', $jobNumber)->delete('Trans_JobFinishGoodDT');
    $this->BJGMAS01->where('JobNumber', $jobNumber)->delete('Trans_JobFinishGoodHD');
