...
class Schedule_Tanggal extends CI_Controller
{
  ...
  public function index()
  {
    ...
    if ($check_permission->num_rows() == 1) {
      ...
      $data['nama_halaman']   = "FORECASH PPIC DAN SALES";
      ...
      $this->load->view('adminx/salesorder/schedule_tanggal_view', $data, FALSE);
  ...
  public function schedule_list()
  {
    ...
    $sql          = "SELECT
                        ...
                        partId,
                        qtySchedule,
                        CAST(tglSchedule AS DATE) AS tglSchedule,
                        ppicApprove
                    FROM
                        tbl_schd
                    WHERE ppicApprove = 'Approved'
                    ORDER BY tglSchedule";
    $query        = $second_DB->query($sql);
    $result       = $query->result();
    $jam          = array();

    foreach (range(0, 60, 10) as $number) {
      $jam[] = $number;
    }

    foreach ($result as $key => $value) {

      $title  = "No. SC: ".$value->noSC." - Part ID: ".$value->partId." - Qty: ".number_format($value->qtySchedule);
      ...
      $data[] = array(
        ...
        "title"       => $title,
        "start_date"  => $value->tglSchedule,
        "end_date"    => $value->tglSchedule

        // "start_date"  => $value->tglSchedule." 09:".$jam[$key].":00",
        // "end_date"    => $value->tglSchedule." 09:".$jam[$key].":00"
      );
    }
  ...
  public function schedule_list_OLD()
  {
    ...
    foreach ($result as $key => $value) {
      ...
      $title = "$value->noSC - $value->partId - $value->qtySchedule";

      $data[] = array(
        "title" => $title,
        "start" => $value->tglSchedule,
        "end" => $value->tglSchedule
      );
    }
