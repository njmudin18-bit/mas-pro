...
{
  var $table          = 'Ms_Allowance';
  var $column_order   = array('AllowanceName', 'AllowanceType', 'Amount', 'Percentage', 'ApplyTo', 'Period', 'EffectiveDate', 'IsActive', 'CreatedDate', 'CreatedBy', null);
  var $column_search  = array('AllowanceName', 'AllowanceType', 'Amount', 'Percentage', 'ApplyTo', 'Period', 'EffectiveDate', 'IsActive', 'CreatedDate', 'CreatedBy');
  var $order          = array('AllowanceID' => 'desc');
  ...
  {
    $this->ABSENSI->select("AllowanceID, AllowanceName, AllowanceType,
                            FORMAT(Amount, 'N2', 'id-ID') AS Amount,
                            Percentage, Period, EffectiveDate, ApplyTo,
                            CASE 
  ...
  {
    $this->ABSENSI->select("AllowanceID, AllowanceName,
	                          CAST(FORMAT(Amount, 'N0', 'id-ID') AS VARCHAR(20)) AS Amount");
    $this->ABSENSI->from($this->table);
    $this->ABSENSI->where('IsActive', 'A');
    $this->ABSENSI->where('AllowanceName <>', 'Uang Makan');
    $query = $this->ABSENSI->get();
