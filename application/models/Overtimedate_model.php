...
class Overtimedate_model extends CI_Model
{
  var $table          = 'Ms_TanggalLembur';
  var $column_order   = array('OvertimeDate', 'OvertimeName', 'Amount', 'IsActive', 'Notes', 'CreatedDate', 'CreatedBy', null);
  var $column_search  = array('OvertimeDate', 'OvertimeName', 'Amount', 'IsActive', 'Notes', 'CreatedDate', 'CreatedBy');
  ...
  {
    $this->ABSENSI->select("Id, OvertimeDate, OvertimeName,
                            FORMAT(Amount, 'N0', 'id-ID') AS Amount,
                            Notes,
                            CASE 
  ...
  {
    $this->ABSENSI->select("Id, OvertimeDate, UPPER(OvertimeName) AS OvertimeName, IsActive,
	                          FORMAT(Amount, 'N0', 'id-ID') AS Amount");
    $this->ABSENSI->from($this->table);
    $this->ABSENSI->where('IsActive', 'A');
    $this->ABSENSI->order_by('OvertimeName', 'ASC');
    $query = $this->ABSENSI->get();
