...
<html lang="en">
  ...
  <body>
                                ...
                                <table id="order-table" class="table table-striped table-bordered table-hover" width="130%">
                                  <thead class="bg-primary text-center">
                                    <tr>
                                      ...
                                      <th class="text-center" width="7%">ALLOWANCE NAME</th>
                                      <th class="text-center" width="7%">ALLOWANCE TYPE</th>
                                      <th class="text-center" width="7%">AMOUNT</th>
                                      <th class="text-center" width="7%">PERCENTAGE</th>
                                      <th class="text-center" width="8%">APPLY TO</th>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form id="RegisterValidation">
              ...
              <div class="form-group row">
                <label class="col-sm-2 col-form-label">Allowance Name</label>
                <div class="col-sm-4">
                  <input type="text" name="AllowanceName" id="AllowanceName" class="form-control text-capitalize" required="required" autocomplete="off" placeholder="Contoh: Tunjangan Makan">
                  <span class="help-block"></span>
                </div>
                <label class="col-sm-2 col-form-label">Allowance Type</label>
                <div class="col-sm-4">
                  <select name="AllowanceType" id="AllowanceType" class="form-control" onchange="handleAllowanceType()">
                    <option value="" selected readonly>-- Pilih --</option>
              ...
              <div class="form-group row">
                <label class="col-sm-2 col-form-label">Apply to</label>
                <div class="col-sm-4">
                  <select name="ApplyTo" id="ApplyTo" class="form-control">
                    <option value="" selected readonly>-- Pilih --</option>
                    <option value="ALL EMPLOYEE">ALL EMPLOYEE</option>
                    <option value="WEEKLY EMPLOYEE">WEEKLY EMPLOYEE</option>
                    <option value="MONTHLY EMPLOYEE">MONTHLY EMPLOYEE</option>
                  </select>
                  <span class="help-block"></span>
                </div>
    ...
    <script type="text/javascript">
      ...
      function handleAllowanceType() {
        var type = $('#AllowanceType').val();
      ...
      function openModal() {
        ...
        $('.help-block').empty();
        $('#modal').modal('show');
        $('.modal-title').text('Tambah Tunjangan');
      }
      ...
      function edit(id) {
        ...
        $.ajax({
          url: "<?php echo base_url(); ?>allowance/allowance_edit/" + id,
          type: "GET",
          dataType: "JSON",
          success: function(data) {
            if (data.status == 'forbidden') {
              ...
              $('[name="kode"]').val(data.AllowanceID);
              $('[name="AllowanceName"]').val(data.AllowanceName);
              $('[name="AllowanceType"]').val(data.AllowanceType);
              $('[name="Amount"]').val(formatRupiahDecimal(data.Amount));
              $('[name="Percentage"]').val(data.Percentage);
              $('[name="ApplyTo"]').val(data.ApplyTo);
      ...
      function openModalDelete(id) {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: '<?php echo base_url(); ?>allowance/allowance_deleted/' + id,
              type: 'DELETE',
              error: function() {
      ...
      function save() {
        ...
        if (save_method == 'add') {
          url = "<?php echo base_url(); ?>allowance/allowance_add";
        } else {
          url = "<?php echo base_url(); ?>allowance/allowance_update";
        }
      ...
      $(document).ready(function() {
        ...
        table = $('#order-table').DataTable({
          ...
          "ajax": {
            "url": "<?php echo base_url(); ?>allowance/allowance_list",
            "type": "POST",
          },
          fixedColumns: {
            left: 3
          ...
          "aoColumns": [
            ...
            { "ALLOWANCE NAME": "ALLOWANCE NAME" , "sClass": "text-left", "width": "50px" },
            { "ALLOWANCE TYPE": "ALLOWANCE TYPE" , "sClass": "text-left", "width": "50px" },
            { "AMOUNT": "AMOUNT" , "sClass": "text-right", "width": "50px" },
            { "PERCENTAGE": "PERCENTAGE" , "sClass": "text-right", "width": "50px" },
            { "APPLY TO": "APPLY TO" , "sClass": "text-left", "width": "80px" },
        ...
        $("#DeductionName, #DeductionType, #Amount, #Percentage, #EffectiveDate, #ApplyTo, #Periode, #IsActive").change(function(){
