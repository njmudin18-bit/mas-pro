          ...
          <div class="modal-body">
            <form id="RegisterValidation">
              ...
              <div class="form-group row">
                ...
                <div class="col-sm-4">
                  <input type="date" name="StartDate" id="StartDate" class="form-control" required="required" autocomplete="off">
                  <span class="help-block"></span>
                </div>
                <label class="col-sm-2 col-form-label">End Date</label>
                <div class="col-sm-4">
                  <input type="date" name="EndDate" id="EndDate" class="form-control" required="required" autocomplete="off">
                  <span class="help-block"></span>
                </div>
              </div>
              <div class="form-group row">
                <label class="col-sm-2 col-form-label">Payday</label>
                <div class="col-sm-4">
                  <input type="date" id="Payday" name="Payday" class="form-control" autocomplete="off">
                  <span class="help-block"></span>
                </div>
                <label class="col-sm-2 col-form-label">Periode Ke</label>
                <div class="col-sm-4">
    ...
    <script type="text/javascript">
      $(function() {
        ...
        function cb(start, end) {
            // Menggunakan .val() untuk elemen <input>
        ...
        $('#tanggal').daterangepicker({
            // maxDate: new Date(), // Menghilangkan ini jika Anda ingin mencakup seluruh tahun
            startDate: start,
            endDate: end,
            ranges: {
                'Today': [moment(), moment()],
                ...
                'This Year': [moment().startOf('year'), moment().endOf('year')] // Tambahkan ini agar sesuai dengan default Anda
            },
            locale: {
                format: 'YYYY-MM-DD'
            }
            // Pastikan Anda meneruskan CB sebagai argumen kedua ke daterangepicker(), 
            // bukan menempatkannya di dalam objek konfigurasi.
        }, cb);
        
        // Panggil callback untuk mengisi input saat halaman dimuat
        cb(start, end);
      });
    </script>
    <script type="text/javascript">
      ...
      function openModal() {
        ...
        $('.help-block').empty();
        $('#modal').modal('show');
        $('.modal-title').text('Tambah Periode Gaji');
      }
      ...
      function edit(id) {
        ...
        $.ajax({
          url: "<?php echo base_url(); ?>periode_gaji/periodegaji_edit/" + id,
          type: "GET",
          dataType: "JSON",
          success: function(data) {
            if (data.status == 'forbidden') {
              ...
              $('[name="Payday"]').val(data.Payday);
              $('[name="Cycle"]').val(data.Cycle);
              $('#modal').modal('show');
              $('.modal-title').text('Edit Periode Gaji');
              $('#btnSave').text('Update');
            }
          },
      ...
      function openModalDelete(id) {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: '<?php echo base_url(); ?>periode_gaji/periodegaji_deleted/' + id,
              type: 'DELETE',
              error: function() {
      ...
      function save() {
        ...
        if (save_method == 'add') {
          url = "<?php echo base_url(); ?>periode_gaji/periodegaji_add";
        } else {
          url = "<?php echo base_url(); ?>periode_gaji/periodegaji_update";
        }
      ...
      $(document).ready(function() {
        ...
        table = $('#order-table').DataTable({
          ...
          "ajax": {
            "url": "<?php echo base_url(); ?>periode_gaji/periodegaji_list",
            "type": "POST",
            "data": function(data) {
          ...
          "aoColumns": [
            ...
            { "PAYDAY": "PAYDAY" , "sClass": "text-left", "width": "50px" },
            { "START DATE": "START DATE" , "sClass": "text-center", "width": "50px" },
            { "END DATE": "END DATE" , "sClass": "text-center", "width": "50px" },
            { "PERIODE KE": "PERIODE KE" , "sClass": "text-center", "width": "50px" },
        ...
        $("#StartDate, #EndDate, #Payday, #Cycle").change(function(){
