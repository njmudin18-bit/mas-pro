  ...
  <body>
                                ...
                                <table id="order-table" class="table table-striped table-bordered nowrap" width="100%" border="1" cellpadding="0" cellspacing="0">
                                  <thead class="bg-primary text-center">
                                    <tr>
                                      ...
                                      <th class="text-center" width="5%">ABSENSI CODE</th>
                                      <th class="text-center" width="7%">ABSENSI NAME</th>
          ...
          <div class="modal-body">
            <form id="RegisterValidation">
              ...
              <div class="form-group row">
                <label class="col-sm-2 col-form-label">Kode Absensi</label>
                <div class="col-sm-4">
                  <input type="text" name="AbsenceCode" id="AbsenceCode" class="form-control text-uppercase" maxlength="3" required="required" autocomplete="off" placeholder="Contoh: S, I, A">
                  <span class="help-block"></span>
                </div>
                <label class="col-sm-2 col-form-label">Nama Absensi</label>
                <div class="col-sm-4">
                  <input type="text" name="AbsenceName" id="AbsenceName" class="form-control text-capitalize" maxlength="75" required="required" autocomplete="off" placeholder="Contoh: S=Sakit, I=Ijin dan A=Alpa">
                  <span class="help-block"></span>
                </div>
              </div>
              <div class="form-group row">
                ...
                <div class="col-sm-10">
                  <input type="text" name="Description" id="Description" class="form-control" required="required" autocomplete="off" placeholder="Keterangan Tambahan">
                  <span class="help-block"></span>
                </div>
              </div>
            </form>
    ...
    <script>
      ...
      function openModal() {
        ...
        $('.help-block').empty();
        $('#modal').modal('show');
        $('.modal-title').text('Tambah Tipe Absensi');
      }
      ...
      function edit(id) {
        ...
        $.ajax({
          url: "<?php echo base_url(); ?>type_absensi/type_edit/" + id,
          type: "GET",
          dataType: "JSON",
          success: function(data) {
            if (data.status == 'forbidden') {
              ...
              $('[name="AbsenceCode"]').val(data.AbsenceCode);
              $('[name="AbsenceName"]').val(data.AbsenceName);
              $('[name="Description"]').val(data.Description);
              $('#modal').modal('show');
              $('.modal-title').text('Edit Tipe Absensi');
              $('#btnSave').text('Update');
            }
          },
      ...
      function openModalDelete(id) {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: '<?php echo base_url(); ?>type_absensi/type_deleted/' + id,
              type: 'DELETE',
              error: function() {
      ...
      function save() {
        ...
        if (save_method == 'add') {
          url = "<?php echo base_url(); ?>type_absensi/type_add";
        } else {
          url = "<?php echo base_url(); ?>type_absensi/type_update";
        }
      ...
      $(document).ready(function() {
        table = $('#order-table').DataTable({
          ...
          "ajax": {
            "url": "<?php echo base_url(); ?>type_absensi/type_list",
            "type": "POST",
          },
          "aoColumns": [
            { "NO": "NO" , "sClass": "text-right", "width": "50px"},
            { "#": "#" , "sClass": "text-center", "width": "50px"},
            { "ABSENSI CODE": "ABSENSI CODE" , "sClass": "text-center", "width": "50px" },
            { "ABSENSI NAME": "ABSENSI NAME" , "sClass": "text-left", "width": "150px" },
            { "DESCRIPTION": "DESCRIPTION" , "sClass": "text-left", "width": "250px" },
        ...
        $("#AbsenceCode").change(function() {
          $(this).parent().removeClass('has-error');
          $(this).next().empty();
        });

        $("#AbsenceName").change(function() {
          $(this).parent().removeClass('has-error');
          $(this).next().empty();
        });
