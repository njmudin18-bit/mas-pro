...
<html lang="en">
  ...
  <body>
                                ...
                                <table id="order-table" class="table table-striped table-bordered nowrap" width="125%" border="1" cellpadding="0" cellspacing="0">
                                  <thead class="bg-primary text-center">
                                    <tr>
                                      ...
                                      <th class="text-center" width="7%">KEMBALI?</th>
                                      <th class="text-center" width="7%">TANGGAL</th>
                                      <th class="text-center" width="7%">JAM PERGI</th>
                                      <th class="text-center" width="7%">JAM KEMBALI</th>
                                      ...
                                      <th class="text-center" width="10%">SECURITY CHECK PERGI</th>
                                      <th class="text-center" width="10%">SECURITY CHECK PERGI BY</th>
                                      <th class="text-center" width="10%">SECURITY CHECK KEMBALI</th>
                                      <th class="text-center" width="10%">SECURITY CHECK KEMBALI BY</th>
                                    </tr>
                                  </thead>
                                  <tbody></tbody>
                                </table>
          ...
          <div class="modal-body">
            <form id="formData" method="post">
              ...
              <div class="form-group row">
                ...
                <div class="col-sm-4">
                  <select name="Kembali" id="Kembali" class="form-control">
                    <option value="" selected>-- Pilih --</option>
              ...
              <div class="form-group row">
                ...
                <label class="col-sm-2 col-form-label">Jam Pergi</label>
                <div class="col-sm-4">
                  <input type="time" name="JamPergi" id="JamPergi" class="form-control" placeholder="Jam Pergi">
                  <span class="help-block"></span>
                </div>
              </div>
              <div class="form-group row">
                ...
                <div class="col-sm-10">
                  <textarea name="Notes" id="Notes" class="form-control" rows="3" maxlength="255" placeholder="Keterangan ijin keluar"></textarea>
                  <span class="help-block"></span>
                </div>
              </div>
            </form>
    ...
    <script>
      ...
      function securityCheckGo(ID, EmployeeName, EmployeeID)
      {
        // 1. Siapkan data Options Security (sama seperti sebelumnya)
        ...
        // Ubah object menjadi string HTML <option>
        let optionsHtml = '<option value="" disabled selected>Pilih Security Checker</option>';
        for (const [key, value] of Object.entries(securityOptions)) {
        ...
        Swal.fire({
          ...
          icon: 'question',
          html: `
            <div style="text-align:left; margin-bottom:5px; font-weight:bold;">Security Checker:</div>
            <select id="swal-security2" class="swal2-select" style="width:100%; margin: 0 0 15px 0;">
              ${optionsHtml}
            </select>
            
            <div style="text-align:left; margin-bottom:5px; font-weight:bold;">Tanggal Pergi:</div>
            <input type="datetime-local" id="swal-date2" class="swal2-input" style="width:100%; margin: 0;">
          `,
          ...
          focusConfirm: false,
          // Set default tanggal hari ini saat popup muncul
          didOpen: () => {
            const dateInput = Swal.getPopup().querySelector('#swal-date2');
            const today     = new Date().toISOString().split('T')[0];
            dateInput.value = today;
          },

          // Validasi Manual & Pengambilan Data
          preConfirm: () => {
            const securityVal = document.getElementById('swal-security2').value;
            const dateVal     = document.getElementById('swal-date2').value;

            if (!securityVal) {
              Swal.showValidationMessage('Silahkan pilih security checkernya.');
              
              return false;
            }
            if (!dateVal) {
              Swal.showValidationMessage('Silahkan pilih tanggal kembali.');

              return false;
            }

            // Return data sebagai object
            return {
              securityID: securityVal,
              returnDate: dateVal
            }
          }
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: '<?php echo base_url(); ?>ijin_keluar/security_check_go', // Pastikan URL benar
              type: 'POST',
              data: {
                Id: ID,
                Nip: EmployeeID,
                SecurityID: result.value.securityID,
              ...
              success: function(data) {
                if (data.status === 'forbidden') {
                  ...
                  Swal.fire(capitalizeFirstLetter(data.status), data.message, 'info');
                } else {
                  reload_table();
                }
                $("#loading").hide();
      ...
      function securityCheckBack(ID, EmployeeName, EmployeeID)
      {
        // 1. Siapkan data Options Security (sama seperti sebelumnya)
        const securityOptions = {
          '0012025021804': 'AHMAD SUBANDI',
        ...
        Swal.fire({
          ...
          html: `
            <div style="text-align:left; margin-bottom:5px; font-weight:bold;">Security Checker:</div>
            <select id="swal-security" class="swal2-select" style="width:100%; margin: 0 0 15px 0;">
              ${optionsHtml}
            </select>
            
            <div style="text-align:left; margin-bottom:5px; font-weight:bold;">Tanggal Kembali:</div>
          ...
          // Validasi Manual & Pengambilan Data
          preConfirm: () => {
            const securityVal = document.getElementById('swal-security').value;
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: '<?php echo base_url(); ?>ijin_keluar/security_check_back', // Pastikan URL benar
              type: 'POST',
              data: {
                  Id: ID,
                  Nip: EmployeeID,
                  SecurityID: result.value.securityID, // Ambil dari object preConfirm
                  ReturnDate: result.value.returnDate  // Data Tanggal Baru
              },
              dataType: "json",
              beforeSend: function() {
                  $("#loading").show();
      ...
      function approved(ID, Status, Label) 
      {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: '<?php echo base_url(); ?>ijin_keluar/ijin_keluar_approved',
              type: 'POST',
              data: {
                Id: ID,
                isApproved: Status
      ...
      function edit(id) {
        ...
        $.ajax({
          url: "<?php echo base_url(); ?>ijin_keluar/ijin_keluar_edit/" + id,
          type: "GET",
          dataType: "JSON",
          success: function(data) {
            if (data.status == 'forbidden') {
      ...
      function openModalDelete(id) {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: '<?php echo base_url(); ?>ijin_keluar/ijin_keluar_deleted/' + id,
              type: 'DELETE',
              error: function() {
                alert('Something is wrong');
                $("#loading").hide();
      ...
      function save() 
      {
        ...
        if (save_method == 'add') {
          url = "<?php echo base_url(); ?>ijin_keluar/ijin_keluar_add";
        } else {
          url = "<?php echo base_url(); ?>ijin_keluar/ijin_keluar_update";
        }
      ...
      $(document).ready(function() {
        ...
        table = $('#order-table').DataTable({
          ...
          "ajax": {
            "url": "<?php echo base_url(); ?>ijin_keluar/ijin_keluar_list",
            "type": "POST",
            "data": function(data) {
              let DeptShow = [];
          ...
          "aoColumns": [
            ...
            { "KEMBALI?": "KEMBALI?" , "sClass": "text-center", "width": "50px" },
            { "TANGGAL": "TANGGAL" , "sClass": "text-center", "width": "80px" },
            { "JAM PERGI": "JAM PERGI" , "sClass": "text-center", "width": "80px" },
            { "JAM KEMBALI": "JAM KEMBALI" , "sClass": "text-center", "width": "80px" },
            ...
            { "SECURITY CHECK PERGI": "SECURITY CHECK PERGI" , "sClass": "text-center", "width": "80px" },
            { "SECURITY CHECK PERGI BY": "SECURITY CHECK PERGI BY" , "sClass": "text-left", "width": "80px" },
            { "SECURITY CHECK KEMBALI": "SECURITY CHECK KEMBALI" , "sClass": "text-center", "width": "80px" },
            { "SECURITY CHECK KEMBALI BY": "SECURITY CHECK KEMBALI BY" , "sClass": "text-left", "width": "80px" },
          ],
        ...
        $("#DeptID, #EmployeeID, #Keperluan, #Kembali, #Tanggal, #JamPergi, #Notes").change(function(){
