...
<html lang="en">
  ...
  <body>
      ...
      <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form action="" method="post" id="reworkForm">
              ...
              <div class="form-group row mb-1">
                <label class="col-sm-2 col-form-label">Reject</label>
                <div class="col-sm-10 form-error mb-2">
                  <input type="text" name="Reject" id="Reject" class="form-control" required="required" placeholder="Keterangan Reject" autocomplete="off">
                  <span class="help-block"></span>
                </div>
              </div>
    ...
    <script type="text/javascript">
      ...
      function reset_all() {
        $('#reworkForm .form-group .has-error').removeClass('has-error');
        $('#reworkForm')[0].reset();
      ...
      function openModal() {
        ...
        $("#pass_div").show();
        $('#btnSave').text('Save');
        $('#reworkForm')[0].reset(); // reset form on modals
      ...
      function save() 
      {
        var form_data = $("#reworkForm").serialize();

        var url;
        if(save_method == 'add') {
          url = "<?php echo base_url(); ?>rework/save_data";
        } else {
          url = "<?php echo base_url(); ?>rework/update_data";
        }
      ...
      function edit(JobNumbers) 
      {
        ...
        $.ajax({
          url: "<?php echo base_url(); ?>rework/edit_data",
          type: "POST",
          dataType: "JSON",
          data: {
          ...
          success: function(data) {
            if (data.status == 'forbidden') {
              ...
              $('[name="Reject"]').val(data.first.Reject);
      ...
      function hapusRowJumlah(rowId)
      {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: "<?php echo base_url(); ?>rework/hapus_row_jumlah",
              type: "POST",
              dataType: "JSON",
              data: {
                JobNumber: jobNumber,
      ...
      function hapusRowNg(rowId)
      {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: "<?php echo base_url(); ?>rework/hapus_row_ng",
              type: "POST",
              dataType: "JSON",
              data: {
                JobNumber: jobNumber,
      ...
      function hapusAll(jobNumber) 
      {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: '<?php echo base_url(); ?>rework/hapus_all',
              type: 'POST',
              data: {
                JobNumber: jobNumber
              },
      ...
      $(document).ready(function() {
        ...
        table = $('#myTable').DataTable({
          ...
          buttons: [
            {
              ...
              customize: function (doc) {
                ...
                doc.content.unshift(
                    ...
                    {
                        text: 'CONTROL FINISHED GOOD REWORK',
                        bold: true,
                        fontSize: 14,
                        style: 'subheader',
                        alignment: 'center',
                ...
                if (mainTable) {
                    const alignRightCols = [0, 5, 6, 7, 8, 9, 10, 11, 12, 13];
              ...
              filename: function() {
                ...
                return 'Laporan Control Finished Good Rework Periode ' + month + ' ' + year;
              }
            }
          ],
          select: {
          ...
          "order": [],
          "ajax": {
            "url": "<?php echo base_url(); ?>rework/list_data",
            "type": "POST",
            "data": function(data) {
          ...
          "aoColumns": [
            ...
            { "REJECT": "REJECT" , "sClass": "text-left", "width": "150px" },              //15
          ...
          "footerCallback": function (row, data, start, end, display) {
              ...
              api.columns().every(function () {
                  ...
                  var isNumeric = [6, 8, 10, 11, 12, 13, 14].includes(columnIndex);

                  if (isNumeric) {
                      // Ambil hanya baris SUB TOTAL
                      var total = 0;
                      data.forEach(function (rowData) {
                          if (rowData[2]?.toString().toUpperCase() === 'SUB TOTAL') {
                    ...
                    // Hitung persentase berdasarkan subtotal
                    let totalPersen = totalJob > 0 ? (totalWH / totalJob) * 100 : 0;
                    $(column.footer()).html(totalPersen.toFixed(2) + '% ada');
                  } else {
                    $(column.footer()).html('');
                  }
              });

              // Output juga di luar tabel
              if (totalJob > 0) {
        ...
        $('#modalForm').on('shown.bs.modal', function () {
          $('#JobList').select2({
            ...
            allowClear: true,
            ajax: {
                url: '<?php echo base_url(); ?>rework/get_job_number',
                type: 'POST',
                dataType: 'JSON',
                delay: 250,
