                              ...
                              <div class="row d-print-none">
                                ...
                                <div class="col-sm-6 text-left">
                                  <button onclick="show_modal_tambah()" class="btn btn-warning mt-2 me-5" type="button" title="Tambah QR">
                                    <i class="fa fa-qrcode ml-1"></i> Tambah QR
                                  </button>
                                </div>
                              </div>
    ...
    <script type="text/javascript">
      ...
      function tampilkan_data_terpilih() {
        ...
        if (PilihanArray.length == 1 && PilihanArray[0] == 'all') {
          ...
          let PONumberArray = <?php echo $array_nomor_po ?>;
          let PartIDArray   = <?php echo $array_part_id ?>;
          let DateArray     = "<?php echo $array_date ?>";

          show_fifo_card(PONumberArray, PartIDArray, PilihanAll);
        } else {
          console.log(PilihanArray);
          $.ajax({
      ...
      function show_fifo_card(PONumberArray, PartIDArray, DateArray) 
      {
        $.ajax({
          url: "<?php echo base_url(); ?>incoming/get_fifo_card",
          dataType: 'JSON',
          data: {
            PONumber: PONumberArray,
            PartID: PartIDArray,
                ...
                items.push('<div id="preview_fifo_' + Sequents + '" class="col-md-5 preview-fifo">' +
                            '<p class="d-print-none fs-8 font-weight-bold mt-3">' + 
                              ...
                              '<button onclick="hapus_barcode(' + NomorBarcode2 + ');" type="button" title="Hapus Barcode ' + value.BarcodeNumber + '" class="btn btn-danger btn-sm pull-right mb-2">' +
                                '<i class="fa fa-trash"></i>'+
                              '</button>' +
                              '<button onclick="hide_barcode(' + Sequents + ');" type="button" title="Sembunyikan Barcode ' + value.BarcodeNumber + '" class="btn btn-dark btn-sm pull-right mb-2 mr-1">' +
                                '<i class="fa fa-eye"></i>'+
                              '</button>' +
                            '</p>' +
      ...
      $(document).ready(function() 
      {
        $("#loading").hide();
        let FIFO_CARD     = JSON.parse(localStorage.getItem('FIFO_CARD'));
        let PONumberArray = <?php echo $array_nomor_po ?>;//FIFO_CARD.PONumber;
        let PartIDArray   = <?php echo $array_part_id ?>;//FIFO_CARD.PartID;
        let DateArray     = "<?php echo $array_date ?>";//FIFO_CARD.Date;
        show_fifo_card(PONumberArray, PartIDArray, DateArray);
        ...
        table = $('#example').DataTable({
          ...
          "ajax": {
            ...
            "type": "POST",
            "data": function(data) {
              data.PONumber   = <?php echo $array_nomor_po ?>;
              data.PartID     = <?php echo $array_part_id ?>;
              data.Date       = "<?php echo $array_date ?>";
            }
          },
